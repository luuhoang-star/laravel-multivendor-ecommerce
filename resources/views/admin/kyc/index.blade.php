        ...
        <div class="container-xl">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Tất cả yêu cầu KYC</h3>
                </div>
                <div class="card-body">
                        ...
                        <table class="table table-vcenter card-table">
                            ...
                            <tbody>
                                @foreach ($kycRequests as $kycRequest)
                                    <tr>
                                        <td>{{ ($kycRequests->currentPage() - 1) * $kycRequests->perPage() + $loop->iteration }}
                                        </td>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.kyc.show', $kycRequest) }}"
                ...
                @if ($kycRequests->hasPages())
                    <div class="card-footer d-flex align-items-center justify-content-between">
                        <p class="m-0 text-secondary">
                            Hiển thị từ <strong>{{ $kycRequests->firstItem() }}</strong> đến
                            <strong>{{ $kycRequests->lastItem() }}</strong> trong tổng số
                            <strong>{{ $kycRequests->total() }}</strong> kết quả
                        </p>
                        <div class="ms-auto">
                            {{ $kycRequests->links() }}
                        </div>
